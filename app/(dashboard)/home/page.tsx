  ...
  const fetchData = async () => {
    try {
      ...
      const [chartResponse, statsResponse, tableResponse] = await Promise.all([
        fetch(`https://mirtech.whisttle.cloud/stats/charts?period=${timePeriod}`),
        fetch(`https://mirtech.whisttle.cloud/stats/summary?period=${timePeriod}`),
        fetch(`https://mirtech.whisttle.cloud/all?period=${timePeriod}&limit=1000&skip=0`) // Start from offset 0
      ]);
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const endpoint = isSearchMode 
        ? `https://mirtech.whisttle.cloud/products/search?query=${encodeURIComponent(productSearchQuery)}&skip=${currentOffset}`
        : `https://mirtech.whisttle.cloud/all?period=${timePeriod}&limit=1000&skip=${currentOffset}`;
  ...
  const handleProductSearch = async (searchQuery: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://mirtech.whisttle.cloud/products/search?query=${encodeURIComponent(searchQuery)}&limit=1000&skip=0`
      );
    ...
    const prefetchLongerPeriods = async () => {
      ...
      for (const period of periodsToPreload) {
        ...
        try {
          ...
          fetch(`https://mirtech.whisttle.cloud/stats/charts?period=${period}`).catch(() => {});
          // Prefetch stats endpoint (fire and forget)
          fetch(`https://mirtech.whisttle.cloud/stats/summary?period=${period}`).catch(() => {});
          // Prefetch limited table data (fire and forget)
          fetch(`https://mirtech.whisttle.cloud/all?period=${period}&limit=1000`).catch(() => {});
        } catch (err) {
